...
from relecloud.models import Destination, Review, Purchase


class TestCrearResenaDestino(TestCase):
    """
    RED: Lógica de creación de reseñas para destinos.
    """
    ...
    def test_usuario_autenticado_puede_crear_resena(self):
        """Un usuario autenticado puede enviar una reseña para un destino."""
        self.client.login(username='testuser', password='TestPass123!')
        response = self.client.post(self.url, {
            'rating': 5,
            'title': 'Increíble',
            'comment': 'Una experiencia única',
        })
        self.assertEqual(Review.objects.count(), 1)
        resena = Review.objects.first()
        self.assertEqual(resena.user, self.user)
        self.assertEqual(resena.destination, self.destination)
        self.assertEqual(resena.rating, 5)

    def test_crear_resena_redirige_a_detalle_destino(self):
        """Tras crear una reseña se redirige a la página del destino."""
        self.client.login(username='testuser', password='TestPass123!')
        response = self.client.post(self.url, {
            'rating': 4,
            'title': 'Muy bien',
    ...
    def test_resena_duplicada_devuelve_403(self):
        """Un usuario no puede crear dos reseñas para el mismo destino."""
    ...
    def test_resena_con_rating_invalido_no_se_guarda(self):
        """Un rating fuera de rango (0 o 6) no debe guardarse."""
        self.client.login(username='testuser', password='TestPass123!')
...
class TestVistaDetalleDestinoResenas(TestCase):
    """
    RED: La página de detalle de destino muestra la sección de reseñas correctamente.
    """

    def setUp(self):
        self.user = User.objects.create_user('testuser', password='TestPass123!')
        self.destination = Destination.objects.create(
            name='Luna', description='Satélite natural'
        )
        Purchase.objects.create(user=self.user, destination=self.destination)
    ...
    def test_usuario_autenticado_sin_resena_ve_boton_dejar_resena(self):
        ...
        self.assertContains(response, reverse('review_destination', kwargs={'destination_id': self.destination.pk}))
    ...
    def test_usuario_con_resena_previa_no_ve_boton_nueva_resena(self):
        ...
        self.client.login(username='testuser', password='TestPass123!')
        response = self.client.get(self.url)
        review_url = reverse('review_destination', kwargs={'destination_id': self.destination.pk})
